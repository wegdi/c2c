...
$Edit = $db->Query('Menus',["_id" => $db->ObjectId($param2)], [], 'TEK');
?>
      ...
      <div class="card-body">
        ...
        <form id="feg">
          <div>
            ...
            <input type="text" class="form-control" id="basiInput" name="tr" required value="<?php echo $Edit["tr"]; ?>">
          </div>

          <div  class="mt-3">
            <label for="basiInput" class="form-label"><?php echo $Themes->Translate("TEXT_MENU_NAME_EN"); ?> </label>
            <input type="text" class="form-control" id="basiInput" name="en" required value="<?php echo $Edit["en"]; ?>">
          </div>

          <div  class="mt-3">
            <label for="basiInput" class="form-label"><?php echo $Themes->Translate("TEXT_MENU_ORDER"); ?> </label>
            <input type="number" class="form-control" id="basiInput" name="order" required value="<?php echo $Edit["Order"]; ?>">
          </div>

          <div  class="mt-3">
            <label for="basiInput" class="form-label"><?php echo $Themes->Translate("TEXT_MENU_ICON"); ?> </label>
            <input type="text" class="form-control" id="basiInput" name="icon"  value="<?php echo $Edit["Icon"]; ?>">
          </div>
          ...
          <div class="input-group mt-3">
            ...
            <select class="form-select" id="inputGroupSelect01" name="parent_id" required>
              ...
              <?php foreach ($Menus as $key => $MenusGet): ?>
                <?php $sel = ($MenusGet['_id'] == $Edit["Parent_ID"]) ? 'selected' : ''; ?>
                <option <?php echo  $sel; ?> value="<?php echo $MenusGet['_id']?>" <?php echo $sel; ?>><?php echo $MenusGet[LANGUAGES_GET_DIL]; ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          ...
          <div class="mt-4">
            ...
            <select required multiple="multiple" name="authority[]" id="multiselect-header">
              ...
              <?php foreach ($Authority as $key => $AuthorityGet): ?>
                <?php if (is_array($Edit["Authority"])): ?>
                  <option  <?php if (in_array($AuthorityGet['_id'], $Edit["Authority"])) { echo 'selected';}else {} ?> value="<?php echo $AuthorityGet['_id']->__toString(); ?>"><?php echo $AuthorityGet["Name"]; ?></option>
                  <?php else: ?>
                    <option  value="<?php echo $AuthorityGet['_id'];?>"><?php echo $AuthorityGet["Name"]; ?></option>
